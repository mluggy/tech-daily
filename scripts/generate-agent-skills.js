...
const skills = [
  ...
  },
  {
    ...
    description:
      ...
      "Auth is OPTIONAL — all endpoints accept anonymous calls — but this skill walks the full flow for agents that prefer to authenticate.",
    whenToUse:
    ...
    body: [
      "# Use agent auth",
      "",
      `Walkthrough for obtaining and using an OAuth credential against ${config.title}. Anonymous calls are always accepted; run this skill only when an agent needs an explicit bearer token.`,
      "",
      "## When to use",
      "",
      `When an agent needs an explicit bearer token (audit logging, per-token quotas, or a strict MCP client that requires OAuth). Skip this skill if anonymous access is acceptable — every read endpoint on ${config.title} accepts unauthenticated calls.`,
      ...
      "## Step 1 — Discover",
      "",
      "Probe `GET /agent/auth` for a `WWW-Authenticate: Bearer resource_metadata=…` 401 challenge, or fetch the metadata directly:",
      "",
      "- `GET /.well-known/oauth-protected-resource` (RFC 9728 PRM)",
      "- `GET /.well-known/oauth-authorization-server` (RFC 8414 AS metadata, includes `agent_auth` block)",
      "- `GET /.well-known/openid-configuration` (OIDC discovery)",
      "- `GET /auth.md` (full WorkOS-spec prose walkthrough)",
      "",
      "## Step 2 — Register (RFC 7591)",
      "",
      "```bash",
      ...
      "",
      "# 201 Created — returns client_id=\"public\", client_secret=null,",
      "# token_endpoint_auth_method=\"none\"",
      "```",
      "",
      "## Step 3 — Claim a credential",
      "",
      "Pick the flavor that fits the agent:",
      "",
      ...
      "```",
      "",
      "Tokens are JWS (EdDSA when `SIGNING_PRIVATE_KEY` is configured, HS256 otherwise). Verify against the JWKS at `/oauth/jwks.json`. TTL = 1 hour.",
      "",
      "## Step 5 — Revoke (RFC 7009)",
      "",
      "```bash",
      "curl -X POST /oauth/revoke -d 'token=<access_token>&token_type_hint=access_token'",
      "# 200 OK (empty body — tokens are stateless, revocation is a courtesy ack)",
      "```",
      "",
      "## Errors",
      "",
      "- `401 unauthorized` from `/agent/auth` — discovery challenge, follow the `WWW-Authenticate: Bearer resource_metadata=…` header to the PRM.",
      "- `400 invalid_grant` — PKCE verifier mismatch or expired code.",
      "- `400 unsupported_grant_type` — only `authorization_code`, `client_credentials`, `refresh_token`, and `urn:ietf:params:oauth:grant-type:jwt-bearer` are advertised.",
      "",
      "## See also",
      "",
